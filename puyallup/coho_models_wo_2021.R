...
# Anomaly models redoing wo 2021
...
ggsave(here("puyallup","output","coho_covariates_correlation_anomaly_wo_2021.png"),width = 10, height = 10)
...
ggsave(here("puyallup","output","puyallup_temperature_anomaly_wo_2021.png"), width = 7, height = 5, units = "in", dpi = 300)  


covariates_coho1_puyallup_w_temp <- arrange(data_wo_2021,doy) %>%
  filter(doy >90 & doy <= 160) %>%
...
subset_coho_summer_perhour <- arrange(data_wo_2021,doy) %>%
  filter(doy > 90 & doy <= 160) %>%
...
write.csv(out.tab_anomaly_puyallup.ordered, 
          file = here("puyallup",
                      "output","model_selection_puyallup_coho_anomaly_wo_2021.csv"))
...
save(puyallup_coho_best_model, file = here("puyallup",
                                           "output","puyallup_coho_best_model_anomaly_wo_2021.RData"))
...
write.csv(riv_anomaly_puyallup[order(riv_anomaly_puyallup$relative_importance, decreasing = TRUE),], 
          file = here("puyallup",
                      "output","puyallup_coho_relative_importance_anomaly_wo_2021.csv"))
...
ggsave(here("puyallup","output","coho1_puyallup_prediction_w_hatchery_anomaly_wo_2021.png"), width = 6, height = 8, units = "in", dpi = 300)  
...
ggsave(here("puyallup","output","coho1_puyallup_residuals_anomaly_wo_2021.png"), width = 8, height = 6, units = "in", dpi = 300)  
