...
data_day_night %>% 
  filter(doy>100, doy <= 200, year == 2015) %>% 
  ggplot() +
  # geom_line(aes(x = doy, y = resid, group = as.factor(year), col = "temperature \n residuals"),alpha = 0.5) +
  # geom_line(aes(x = doy, y = temp, group= as.factor(year), col = "temperature"),alpha = 0.5)+
...
ggsave(here("skagit","output","coho_covariates_correlation_anomaly.png"),width = 10, height = 10)
...
covariates_coho1_skagit_night <- arrange(data_day_night,doy) %>%
  ...
  dplyr::select(year,doy, daytime_category, flow_anomaly,
                ...
                temp_diff, flow_diff, 
                # photo_diff, 
                coho1_hatchery_perhour_diff, trap) %>% 
...
for(i in (1+number_done):length(list_combinations)){
  ...
  out=data.frame(c=name, 
                 ...
                 flow_difference = flow_difference, temp_anomaly = temp_anomaly,
                 # photo_difference = photo_difference,
                 # lunar_phase = lunar_phase, 
                 hatchery = hatchery, season = season,
                 logLik=fit$logLik, AICc=fit$AICc, num.param=fit$num.params,
                 num.iter=fit$numIter, converged=!fit$convergence,
                 stringsAsFactors = FALSE)
  print(out)
  out_riv <- rbind(out,out_riv)
  #save the estimates of the covariates in dataframe
  out2 = data.frame(temperature_difference = ifelse(temperature_difference==1, fit$par$U[which(covariates == 4),], NA),
                    ...
                    # photo_difference = ifelse(photo_difference == 1, fit$par$U[which(covariates == 6),], NA),
                    season = ifelse(season == 1, fit$par$U[which(covariates == 3),], NA),
...
write.csv(out.tab_anomaly.ordered, 
          file = here("skagit",
                      "output","model_selection_skagit_coho_anomaly.csv"))



#best model

skagit_coho_best_model <- fits_anomaly[[which.min(sapply(fits_anomaly, function(x) x$AICc))]]

#save

save(skagit_coho_best_model, file = here("skagit",
                                         "output","skagit_coho_best_model_anomaly.RData"))
...
out=data.frame(c=name, 
               ...
               temp_anomaly = temp_anomaly, season = season,
               flow_difference = flow_difference,
               # lunar_phase = lunar_phase, 
               # resid = resid, photo_difference = photo_difference,
               hatchery = hatchery,
...
riv_anomaly <- data.frame(variable = c(#"resid",
  "temperature difference",
  "flow anomaly",
  "temperature anomaly",
  "season",
...
write.csv(riv_anomaly[order(riv_anomaly$relative_importance, decreasing = TRUE),], 
          file = here("skagit",
                      "output","skagit_coho_relative_importance_anomaly.csv"))
...
ggplot(data = predict_coho1_skagit$pred)+
  ...
  geom_line(aes(x = doy, y = estimate, color = "wild, predicted"))+
  geom_point(aes(x = doy, y = y, color = "wild, observed"), size = 0.2, alpha = 0.8)+
  geom_ribbon(aes(x = doy, ymin = `Lo 95`, ymax = `Hi 95`), alpha = 0.2)+
  geom_line(data = predict_coho1_skagit$pred, aes(x = doy, y = coho1_hatchery_perhour_diff,
...
ggsave(here("skagit","output","coho1_skagit_prediction_w_hatchery_anomaly.png"), width = 6, height = 8, units = "in", dpi = 300)  
...
ggsave(here("skagit","output","coho1_skagit_residuals_anomaly.png"), width = 8, height = 6, units = "in", dpi = 300)  
